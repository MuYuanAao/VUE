...
export default {
  ...
  methods: {
    ...
    //更新一个matter
    updateMatter(id,title){
      this.matters.forEach((matter) => {
            if(matter.id === id) matter.title = title
      })
    },
    //删除一个matter
    deleteMatter(id){
  ...
  mounted(){
    this.$bus.$on('checkMatter',this.checkMatter)
    this.$bus.$on('deleteMatter',this.deleteMatter)
    this.$bus.$on('updateMatter',this.updateMatter)
  },
  beforeDestroy(){
    this.$bus.$off('checkMatter')
    this.$bus.$off('deleteMatter')
    this.$bus.$off('updateMatter')
  },
}
</script>
